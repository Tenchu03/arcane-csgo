...
namespace arcane::core::features {
	void auto_bunny_hop( sdk::ifaces::user_cmd* cmd ) {
		...
		if ( cmd->buttons & CONV_ENUM_TYPE( std::int32_t, sdk::enums::button::button_jump ) ) {
			if ( !( local_player->get_flags( ) & CONV_ENUM_TYPE( std::int32_t, sdk::enums::flags::flags_on_ground ) ) )
